...
4.  **Content Discovery**: Using **gobuster** I found:
...
5.  **Login**: Using the username `R1ckRul3s` and password `Wubbalubbadubdub`, I logged into `http://target-ip/login.php`. This gave a command panel:
   
    ![login.php](Images/pickle-rick/login-page.png)
    
    ![portal.php](Images/pickle-rick/command-panel.png)

6.  **Commands that Led to Finding the 3 Flags (Ingredients)**:
    - `whoami` --> **`www-data`**
